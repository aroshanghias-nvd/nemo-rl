...
INPUT_PATH = "/lustre/fsw/portfolios/llmservice/users/smohsenitahe/data/keen_chinese/keensight_multimodal_77k.jsonl"
INPUT_SIZE = 2100

MODEL = "Qwen/Qwen3-VL-235B-A22B-Thinking-FP8"
MAX_GENERATIONS_PER_PROMPT = 2
MAX_TOKENS = 16384
...
def _process_sample(args):
    ...
    for attempt in range(MAX_GENERATIONS_PER_PROMPT):
        ...
        if resp is None:
            continue
        pred_answer = resp["prediction"]
        final_pred_answer = pred_answer.split("</think>")[-1].strip()
        # is_correct = verify_exact_string_match(pred_answer, sample["answer"])
        #is_correct = verify_llm_judge(client, question, pred_answer, sample["answer"])
        is_correct = evaluate_answer_gpt(sample["answer"], final_pred_answer)
        if is_correct == True or attempt == MAX_GENERATIONS_PER_PROMPT - 1:
            result = {**sample, **resp, "attempt": attempt, "correct": is_correct}
            return result, total_tokens, total_latency

    return None, total_tokens, total_latency

def verify_llm_judge_vllm(client, question, pred_answer, answer):
    prompt = f"For the following question: {question}\n\n I have two answers: {answer} and {pred_answer}. Do they have exact same meaning ? and does the answer follow the questions requirements? Please answer just yes or no. Put your answer in \\boxed{...}."
    messages = build_messages(prompt, images=None)
    ...
    res = extract_boxed_answer(resp.choices[0].message.content.strip())
    if 'yes' in res.lower():
        return True
    else:
        return False

def evaluate_answer_gpt(gt: str, pred: str) -> bool:
    """Return evaluation string (e.g., 'CORRECT'/'INCORRECT') from OpenAI call."""
    if openai is None:
        raise RuntimeError("openai package not installed; cannot evaluate.")

    system_prompt = (
        "You are an expert evaluator for question answering tasks. "
        "Return only 'CORRECT' if the candidate answer exactly matches the ground truth "
        "or is an acceptable paraphrase (case-insensitive, ignore punctuation). "
        "Otherwise return 'INCORRECT'. Do NOT output anything except the single word."
    )
    ...
    client = AzureOpenAI(
        api_key="eyJhbGciOiJFUzUxMiIsInR5cCI6IkpXVCJ9..AWu1lUU5nNYRo5DnBTIi-8rWp19XGWSSE13l6lvCwdSep8EQWWsE4OnAWXK4cvqwvsmfx2nM2bjDuB-mbhHqXAvQAIoNAxmNToVCNisfO5B3PqW7klibvcOC6xAWbIlPl6Ow6tuDpK0p9pzKHBPQCQc6G5fMYfK0beGx27VLXt7uJBMva",
    ...
    print(f"Response GPT: {resp.choices[0].message.content.strip()}")
    if 'correct' in resp.choices[0].message.content.strip().lower():
        return True
    else:
        return False
...
def main(input_path, output_path, shard_id, num_shards):
    output_path = output_path + f"_shard_{shard_id}.jsonl"
    task_id = int(os.getenv("SLURM_ARRAY_TASK_ID") or "0")
    port = 18765 + task_id
